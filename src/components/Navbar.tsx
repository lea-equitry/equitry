    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-cream/95 backdrop-blur-sm border-b border-sage-lighter/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
        ...
        <nav className="hidden lg:flex items-center gap-6">
          ...
          <div
            className="relative"
            onMouseEnter={() => openDropdown('location')}
            onMouseLeave={closeDropdown}
          >
            <Link
              href="/location"
              className="font-poppins text-sm font-medium text-sage-darkest hover:text-sage transition-colors tracking-wide flex items-center gap-1 py-5"
            >
              Location
              <span className="text-sage-lighter text-xs">▾</span>
            </Link>
            {dropdown === 'location' && (
              <div className="absolute top-full left-0 bg-cream border border-sage-lighter/30 shadow-lg min-w-[220px] py-2 z-50">
                {categories.map((cat) => (
                  <Link
                    key={cat.slug}
                ...
                <div className="border-t border-sage-lighter/20 mt-1 pt-1">
                  <Link
                    href="/location/tenue-complete"
                    className="flex items-center gap-2 px-4 py-2.5 font-poppins text-sm text-sage-dark hover:bg-sage-lighter/10 hover:text-sage-darkest transition-colors font-medium"
                  >
                    <span>✨</span>
                    Tenue complète
                  </Link>
          ...
          <div
            className="relative"
            onMouseEnter={() => openDropdown('capteurs')}
            onMouseLeave={closeDropdown}
          >
            <Link
              href="/capteurs"
      ...
      {open && (
        <div className="lg:hidden bg-cream border-t border-sage-lighter/30 px-4 py-6 flex flex-col gap-1 max-h-[80vh] overflow-y-auto">
          ...
          <Link href="/location/tenue-complete" className="font-poppins text-sm text-sage-dark py-2 px-4 hover:text-sage border-b border-sage-lighter/20 font-medium" onClick={() => setOpen(false)}>
            ✨ Tenue complète
          </Link>

          <div className="my-2 border-t border-sage-lighter/20" />
          <Link href="/abonnement" className="font-poppins text-sm font-medium text-sage-darkest py-2 px-2 hover:text-sage border-b border-sage-lighter/20" onClick={() => setOpen(false)}>Abonnement</Link>
          <Link href="/occasion" className="font-poppins text-sm font-medium text-sage-darkest py-2 px-2 hover:text-sage border-b border-sage-lighter/20" onClick={() => setOpen(false)}>Occasion</Link>

          <p className="font-poppins text-xs font-semibold uppercase tracking-widest text-sage-light px-2 mt-2 mb-1">Capteurs</p>
          <Link href="/capteurs/tension-de-renes" className="font-poppins text-sm text-sage-dark py-2 px-4 hover:text-sage border-b border-sage-lighter/10" onClick={() => setOpen(false)}>📊 Tension de rênes</Link>
          <Link href="/capteurs/tapis-de-pression" className="font-poppins text-sm text-sage-dark py-2 px-4 hover:text-sage border-b border-sage-lighter/10" onClick={() => setOpen(false)}>🗺️ Tapis de pression</Link>
          <Link href="/capteurs/equisense" className="font-poppins text-sm text-sage-dark py-2 px-4 hover:text-sage border-b border-sage-lighter/10" onClick={() => setOpen(false)}>📱 Equisense</Link>

          <div className="my-2 border-t border-sage-lighter/20" />
          <Link href="/blog" className="font-poppins text-sm font-medium text-sage-darkest py-2 px-2 hover:text-sage border-b border-sage-lighter/20" onClick={() => setOpen(false)}>Blog</Link>
          <Link href="/contact" className="font-poppins text-sm font-medium text-sage-darkest py-2 px-2 hover:text-sage border-b border-sage-lighter/20" onClick={() => setOpen(false)}>Contact</Link>
          <a href="https://equitry-location.fr/" className="btn-primary mt-4 text-center text-xs">Réserver</a>
        </div>
      )}
    </header>
  )
